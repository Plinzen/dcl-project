...
public class CommentDao implements ICommentDao {
	...
	public List<Comment> getForUser(Account user) throws DatabaseException {
		Query q = mEntityManager.createQuery("Select c from Comments c where c.account = ?1 order by c.minute");
	...
	public List<Comment> getForGame(Game game) throws DatabaseException {
		Query q = mEntityManager.createQuery("Select c from Comments c where c.game = ?1 order by c.minute");
		q.setParameter(1, game);
